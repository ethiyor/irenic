...
- A local OS-exclusive lock serializes scheduled runs and analyst mutations on the single host. Durable scheduler state survives restart; incomplete runs stop scheduling for operator review. The process allows an in-progress run up to 270 seconds to finish during graceful shutdown. A forced kill may leave a lock and requires investigation, never automatic resend.
